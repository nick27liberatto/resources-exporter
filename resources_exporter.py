...
#Ping
regex_get_pkg = "ping -c 5 -i 0.2 %s | grep -Po '(?<=received, )\w+' | tr -d '\n'"
...
def fetch_data(vm_services_qnt):
    ...
    for i in range(vm_services_qnt):
        ...
        #Authentication
        host = vm_ips[i]
        user = "infra" # optional: os.popen(get_local_user).read()
        ...
        try:
            ...
            #Get rows length
            ports_qnt = os.popen(regex_ports_qnt%(host)).read()
            
            #Get Ports/Service Line by Line
            for r in range(int(ports_qnt)):
                row = r + 1
